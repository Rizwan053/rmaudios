                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header">
@if(session('update'))
    <div class="p-3 mb-2 bg-secondary text-white text-center">{{session('update')}}</div>
@endif
@if(session('success'))
    <div class="p-3 mb-2 bg-secondary text-white text-center">{{session('success')}}</div>
@endif
@if(session('delete'))
    <div class="p-3 mb-2 bg-secondary text-white text-center">{{session('delete')}}</div>
@endif
                            <a class="btn pull-right" href="{{route('posts.create')}}">Add New Post</a>
...
@foreach($posts as $post)
                                        ...
                                        <td>{{$post->title ? str_limit($post->title, 50) : 'Not Available'}}</td>
